        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == 'add') {
                    ...
                    hisoka.sendMessage(anu.id, { image: { url: buff }, contextInfo: { mentionedJid: [num] }, caption: `Welcome To ${metadata.subject} @${num.split("@")[0]}` })
                } else if (anu.action == 'remove') {
                	let buff2 = await getBuffer (`https://hardianto.xyz/api/goodbye4?profile=${ppuser}&name=${num.split("@")[0]}`)
                    hisoka.sendMessage(anu.id, { image: { url: buff2 }, contextInfo: { mentionedJid: [num] }, caption: `@${num.split("@")[0]} Leaving To ${metadata.subject}` })
                }
            }
        } catch (err) {
            console.log(err)
